...
| 文件 | 改动 |
|---|---|
| `ultralytics/data/base.py` | `__init__` 新增 `_ratio_mask/_blur_mask/_compose_mask` 初始化；`set_epoch` 重构为 4 掩码重建（slice + ratio_pad + blur + compose）；`_ratio_at` 未选中 `skip_pad` 走整图；`_blur_at` 未选中跳过 `_apply_motion_blur`；`_compose_at` 未选中组 `return self._origin_at(index, base)` |
