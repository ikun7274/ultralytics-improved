...
class BaseTrainer:
    ...
    def resume_training(self, ckpt):
        ...
        try:
            ckpt_path = Path(self.resume) if isinstance(self.resume, (str, Path)) else None
            ckpt_path_str = str(ckpt_path) if ckpt_path is not None else "<in-memory ckpt>"
        except Exception:
            ckpt_path_str = repr(self.resume)
        LOGGER.warning(
            f"[resume_security] 正在从 '{ckpt_path_str}' 加载 checkpoint: "
            "ultralytics 通过 weights_only=False 反序列化完整 ckpt (保留 optimizer/scheduler/EMA)。"
            "请确认该 .pt 仅来源于你本人训练, 不要从不可信来源加载 — 否则可能触发任意代码执行。"
        )
