...
def _motion_blur_kernel(length: float, angle: float) -> np.ndarray:
    ...
    size = max(3, int(length) if int(length) % 2 == 1 else int(length) + 1)
...
class BaseDataset(Dataset):
        ...
        if im is None:  # not cached in RAM
            ...
            if self.augment and self.cache != "ram":
                ...
                if getattr(self, "slice_transform", None) is None:
                    # Without slicing, load_image's index is the dataset index, so buffer maintenance stays here.
                    # With slicing, get_image_and_label centrally manages the buffer with dataset (expanded) indices.
    ...
    def _blur_at(self, index: int, long: bool = False) -> dict[str, Any]:
        ...
        # slice_save_annotated, capped by slice_save_max, deduplicated per (image, tier) across epochs.
        ...
        if sdir and st is not None:
            ...
            key = ("blur", tier, index)
            if key not in self._blur_saved_keys and (st.save_max == 0 or self._blur_saved < st.save_max):
                ...
                cv2.imwrite(str(cdir / f"blur_{tier}_img{index}_{self._blur_saved:05d}_n{len(boxes)}.jpg"), img)
                self._blur_saved += 1
    ...
    def _ratio_at(self, index: int) -> dict[str, Any]:
        ...
        cls = np.asarray(lb.get("cls", np.empty((0, 1))), dtype=np.float32)
        ...
        # Annotated per slice_save_annotated, capped by slice_save_max, deduplicated per (image) across
        # epochs/mix visits. Disabled by default (empty dir).
        ...
        if sdir and st is not None:
            ...
            key = ("ratio", index)
            if key not in self._ratio_saved_keys and (st.save_max == 0 or self._ratio_saved < st.save_max):
                cdir.mkdir(parents=True, exist_ok=True)
                img = big
                if st.save_annotated and len(boxes):
                    img = big.copy()
                    ...
                    for b, c in zip(boxes, np.asarray(cls).reshape(-1)):
                        ...
                        cv2.putText(img, f"cls{int(c)}", (x0, max(0, y0 - 4)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                cv2.imwrite(str(cdir / f"ratio_img{index}_{self._ratio_saved:05d}_n{len(boxes)}.jpg"), img)
                self._ratio_saved += 1
    ...
    def _compose_at(self, index: int) -> dict[str, Any]:
        ...
        # n_per mirrors get_image_and_label (5 = 4 slices + 1 origin, plus 1 ratio / 2 blur each if enabled),
        # so the compose block (index >= n_origin*n_per) maps back to the correct group of 4 originals.
        ...
        # slice_save_annotated, capped by slice_save_max, deduplicated per (group) across epochs/mix visits.
        st = getattr(self, "slice_transform", None)
        if st is not None and getattr(self, "compose_save", False):
            ...
            if cdir is not None:
                ...
                key = ("compose", group)
                if key not in self._compose_saved_keys and (st.save_max == 0 or self._compose_saved < st.save_max):
                    cdir.mkdir(parents=True, exist_ok=True)
                    img = big
                    if st.save_annotated and len(bboxes):
                        ...
                        for b, c in zip(bboxes, np.asarray(cls).reshape(-1)):
                            ...
                            cv2.putText(img, f"cls{int(c)}", (x0, max(0, y0 - 4)),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    cv2.imwrite(str(cdir / f"compose_g{group}_{self._compose_saved:05d}_n{len(bboxes)}.jpg"), img)
    ...
    def get_image_and_label(self, index: int, count_slice: bool = True) -> dict[str, Any]:
        ...
        if keep_origin:
            ...
            # (keep_blur) -> len = n_per*N, with n_per = 5/6/7/8. The original samples (sub == 4)
            ...
            n_per = 5 + (1 if keep_ratio else 0) + (2 if keep_blur else 0)
            ...
            is_blur_short = is_blur_long = False
        if is_ratio:
            return self._ratio_at(img_index)  # in-memory aspect-ratio-padded image (lazy, self-manages buffer)
        if is_blur_short:
            return self._blur_at(img_index, long=False)  # in-memory short-tier motion blur (lazy)
        if is_blur_long:
            return self._blur_at(img_index, long=True)  # in-memory long-tier motion blur (lazy)
        ...
        if slice_t is not None and self.augment and not is_origin and random.random() < slice_mix_ratio:
            ...
            im = imread(f, flags=self.cv2_flag)  # original-resolution image (BGR)
            if im is None:
                raise FileNotFoundError(f"Image Not Found {f}")
            if im.ndim == 2:
                im = im[..., None]
    ...
    def __len__(self) -> int:
        ...
        if getattr(self, "slice_all_tiles", False) and getattr(self, "slice_transform", None) is not None:
            ...
            if bool(getattr(self, "slice_keep_origin", False)):
                n = n * 5  # 4 sliced tiles + 1 un-sliced original image per original image
                if bool(getattr(self, "ratio_pad_keep", False)):
                    n += len(self.labels)  # + 1 aspect-ratio-padded image per original image -> x6
                if bool(getattr(self, "blur_keep", False)):
                    n += 2 * len(self.labels)  # + 2 motion-blurred images (short+long) per original -> x8
                if bool(getattr(self, "compose_keep", False)):
