...
class Mosaic(BaseMixTransform):
    ...
    def _save_mosaic(self, labels: dict[str, Any]) -> None:
        ...
        cv2.imwrite(str(self.save_dir / f"mosaic_{self._saved:05d}_n{n_inst}.jpg"), img)
        self._saved += 1
...
class OnlineSlice(BaseTransform):
    ...
    def __init__(
        ...
        self._bg_count = 0
        self._saved = 0
        # Number of times the slicing branch was actually entered (regardless of emission); useful to verify
        # slice_mix_ratio (fraction of samples going through slicing) and for debugging.
        self._n_calls = 0
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace):
    ...
    if slice_enabled:
        dataset.slice_transform = OnlineSlice(
            ...
            save_max=int(getattr(hyp, "slice_save_max", 0)),
