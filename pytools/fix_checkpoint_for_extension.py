...
def fix_checkpoint_for_extension(
    ...
    # 8) 回读验证修补是否生效
    ckpt_check = torch.load(target, map_location="cpu", weights_only=False)
    check_epochs = None
    for key in ("train_args", "args"):
...
def main():
    ...
    parser.add_argument("--ckpt", help="last.pt 的路径（必填）", default=r"C:\Users\ASUS\Desktop\ultralytics-improved\runs\exp-2\weights\last.pt")
    parser.add_argument("--epochs", type=int, help="新的总训练轮数（必须大于已完成轮数）",default=10)
