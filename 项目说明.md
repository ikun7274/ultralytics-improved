...
- **发射策略**：`slice_all_tiles=True` 时每张原图的 4 片子图**全部参与训练**（基础区段固定 4N，`_n_per` 不再被 keep_origin 撑成 5）。
- **保留原图（`slice_keep_origin`，独立开关）**：额外把 1 张未切片原图放进样本池（提供全图上下文）。它已从切片机制中拆出——不再影响 `n_per`，而是独立的 `[4N, 5N)` 区段（`_origin_at`），开关不受 `slice_prob=0` 影响；但切片关闭时由 `_keep_origin_on()` 自动抑制（原图本来就在池里，再保留即重复）。
...
### 3.7 在线增强内存优化（第 2 epoch 卡住的根因与手段）
...
- `ultralytics/data/build.py:382` 显式设置 `prefetch_factor=2`（PyTorch 默认 2/None），8 workers 下总预取深度 = 8×2 = 16 batch ≈ 64 个样本的增强状态同时在内存，峰值达 6~19 GB（视 batch 张量大小）；
...
| `slice_mix_ratio` 降到 0.5~0.7 | train.py 一行 | 一半切片位不读原图不切片 |
...
| `slice_mix_ratio` 无法手动控制 | default.yaml 已注册但 train.py 未暴露 → train.py 切片区新增 `slice_mix_ratio=1.0` 参数行 |
