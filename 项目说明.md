...
4 张原图 → 在线切片（16 切片，由 `slice_ratio` 按 epoch 决定哪些原图走切片）+ 保留原图（4）+ 在线合成（1 张 2×2 大图）+ 在线比例调整（4）+ 在线运动模糊（8，短/长各 2）= **33 张混合样本池**，一起进入 Mosaic 采样，mosaic 从中取 4 张拼成 1280×1280 训练。
...
- 方案 A 中"被拒背景片退回原图"正是这一原则的体现——不丢样本位，换成信息更丰富的原图。
...
- 只有「切片」相关才用 `slice_` 前缀（`slice_overlap_ratio`、`slice_all_tiles`、`slice_ratio`…）；
...
原 `slice_mix_ratio`（per-sample 独立抛硬币，概率近似）已升级为 **`slice_ratio`**：每个 epoch 开始时由 `trainer.py` 调用 `dataset.set_epoch(epoch)` 重建掩码，**精确选 `round(x × N)` 张原图**走在线切片，其余原图整图进池。
...
#### 3.1.3 背景切片控制 + 方案 A（退回原图）

- `slice_background_ratio` = 背景切片数 / 正切片数；-1 全部背景保留，0 不要背景；配额跨原图累计（`bg_count < pos_count × ratio`）。
- **方案 A（关键设计）**：`emit_all` 下配额不足的背景片**不再裁成空片占位**，而是**退回整张原图**（带全量 bbox）。原因：空片与背景片在训练视角等价（同一块裁剪区域 + 空标签），负样本占比由切片几何锁死（目标稀疏场景可达 ~67%），`background_ratio` 调不动它；退回原图后负样本占比由 `background_ratio` 真实控制（实测 67% → 12.5%），且退回原图 = 隐式过采样正样本。
...
> 注意：方案 A 生效后，被拒背景片位已经会退回原图，`slice_keep_origin` 的整图功能被覆盖（每图可能出现约 3.7 份原图冗余）——**建议方案 A 场景下设 `slice_keep_origin=False`**，仅当"目标密集、4 片几乎全有目标"时保留它提供唯一整图上下文。
...
- `_load_image_cached` 现在**直接 imread + worker 内存 LRU**（`slice_raw_cache_size`，原图级，与磁盘缓存无关）；
...
| 空片占位负样本过多 | `emit_all` 下目标稀疏场景空片占比 ~67%，`background_ratio` 调不动 | **方案 A**：被拒背景片退回原图（不再裁空片），负样本占比由 `background_ratio` 真实控制（67% → 12.5%） |
...
- **keep_origin 与切片解耦（独立开关化第二阶段）**：原先 `slice_keep_origin` 与切片强耦合（`n_per=5`、`is_origin=sub==4`、augment.py 在 `slice_prob=0` 时强制关闭）。已重构为第 5 个独立区段：`_n_per` 只返回 4/1、`_segment_bases` 返回 5 边界、新增 `_keep_origin_on()`（无切片自动抑制）与 `_origin_at()`（原图区段路由）、删除死代码 `_compose_count`。全开样本数不变（33），参数名兼容旧配置。
- **slice_mix_ratio → slice_ratio（精确比例化）**：per-sample 独立抛硬币是概率近似（每 epoch 漂移、emit_all 下同图 4 片命运不一致）→ 升级为 epoch 级原图掩码（`set_epoch` 精确选 `round(x×N)` 张，4 片同生共死，无 per-worker 漂移）。
- **功能更改记录**：所有功能更改同步写入 `online_sahi_slice_usage.md`（本文件是整体说明，那是逐条变更日志）。
...
model.train(
    ...
    slice_background_ratio=0.3,            # 背景片/正片比例; 方案A: 被拒背景片退回原图(负样本占比由此值控制)
    ...
    slice_keep_origin=False,               # 方案A下建议关闭(被拒背景片已退回原图, 避免冗余)
    # ---- 在线合成 ----
...
| `ultralytics/data/augment.py` | `OnlineSlice` 类、Mosaic 保存、独立开关装配 |
| `ultralytics/data/base.py` | 索引空间扩展、`set_epoch` 掩码、`_ratio_at` / `_blur_at` / `_compose_at` / `_origin_at`、样本数打印 |
| `ultralytics/engine/trainer.py` | `set_epoch` 钩子、`resume_training` 修补块、`check_resume` 白名单 |
| `ultralytics/cfg/default.yaml` / `__init__.py` | 全部新增参数注册 |
| `train.py` | 训练入口，集中配置全部在线增强 + 续训参数 |
| `online_sahi_slice_usage.md` | 功能更改逐条变更日志 |
