...
> 在原生 Ultralytics（YOLO26）基础上，**不改动原有训练功能**，通过「索引空间扩展」把「在线切片 / 在线合成 / 在线比例调整 / 在线运动模糊 / 在线气象退化」全部搬进内存数据管线，与 Mosaic 无缝衔接；并集成「训练提前终止后的 checkpoint 修补续训」。所有中间产物默认不落盘，除非显式指定保存目录。

---

## 一、整体流程

4 张原图 → 在线切片（16 切片，由 `slice_ratio` 按 epoch 决定哪些原图走切片）+ 保留原图（4）+ 在线合成（1 张 2×2 大图）+ 在线比例调整（4）+ 在线运动模糊（8，短/长各 2）+ 在线气象退化（4，雨/雾/噪声）= **37 张混合样本池**，一起进入 Mosaic 采样，mosaic 从中取 4 张拼成训练尺寸。
...
flowchart LR
    ...
    subgraph POOL["③ 混合样本池 (内存, 不落盘)"]
        C1["16 切片"] --- C2["4 原图"] --- C3["1 合成 2×2"] --- C4["4 比例对齐"] --- C5["8 模糊(短/长)"] --- C6["4 气象退化"]
    end
...
四个比例参数与 `slice_ratio` 完全同构，全部由 `set_epoch(epoch)` 重建掩码；掩码经共享内存 epoch 通道（`_mp_epoch`/`_sync_epoch_masks`）传播到各 DataLoader worker，各进程按确定性 RNG（`crc32(文件列表):epoch:epochs`）重建完全一致的掩码，无 per-worker 漂移（非 "fork 继承"；`persistent_workers=True/False` 均安全）：
...
- **每 epoch 重新随机**：掩码经共享内存 epoch 通道（`_mp_epoch`/`_sync_epoch_masks`）传播到各 DataLoader worker，各进程按确定性 RNG（`crc32(文件列表):epoch:epochs`）重建完全一致的掩码 → 无 per-worker 漂移、比例精确（非 "fork 继承"；`persistent_workers=True/False` 均安全）；
...
#### 3.1.2.1 目标感知切缝（`slice_center_bias`，方案 1）

**问题**：固定 2×2 网格的切缝位置与目标位置无关，目标恰在切缝上会被劈成两半（`slice_center_constraint`/`slice_full_box_only` 只能事后归属或过滤，改变不了"被切"的事实）。

**机制**：切缝按本图目标中心分布微移——目标中心投影到 x/y 轴，竖缝/横缝选在 `[margin, 1-margin]` 区间内目标最稀疏的候选位置；tile 数量/全图覆盖/重叠总量（`2*sw - w`）不变（正 bias 加宽左/上 tile 并等量收窄右/下 tile，切缝精确移动 `bias*w` 且每像素仍至少属于一个 tile）。

| 参数 | 默认 | 作用 |
|---|---|---|
| `slice_center_bias` | False | 目标感知切缝总开关；False=固定网格，完全向后兼容 |
| `slice_bias_margin` | 0.25 | 切缝偏移窗口：只在 `[margin, 1-margin]` 内微移，保证 tile 不过小 |
| `slice_bias_jitter` | 0.05 | 切缝抖动幅度（相对原图边长）：**每张原图每 epoch 只抖一次**——同一图同一 epoch 的 4 片共用同一张网格，跨 epoch 才变化；防切缝固定而过拟合；0=关闭 |

> **切缝的确定性（2026-09-14 修复）**：抖动由 `(epoch, 原图索引)` 派生的确定性 RNG 产生，与全局 `random` 流解耦——
> 因此同一图同一 epoch 的网格**可复现**，且 `slice_all_tiles=True` 下 4 片**必然共享一张网格**（否则 4 片互相矛盾，
> 且并集不再保证覆盖全图）。上表"每像素仍至少属于一个 tile"的承诺现在跨全部 4 片成立。
> ⚠️ 覆盖承诺有边界：把 `slice_bias_jitter` 调到 **≳0.21**（`slice_overlap_ratio=0.2`）时会重新出现覆盖空洞
> （实测 0.22 时 20/200 图有洞、0.25 时 53/200），因为抖动幅度已接近重叠余量；默认 0.05 安全。重叠更大（如 0.4）时余量更宽。

**与现有参数关系**：与 `slice_center_constraint` 互补建议同开（bias 事前躲缝减少劈碎，constraint 事后兜底残余）；`slice_overlap_ratio` 语义不变；`slice_ratio` 未选中的图不偏移；`close_aug_epoch` 回退固定网格；`len` 恒定（仍每图 4 tile）；验证侧 `SliceValDataset` 用固定网格不受影响。

**验证**（20 张小验证集真实标签 + 合成极端分布）：
- 单元：seam 线性偏移、tile 尺寸对称（2800/2000 @ 0.1 bias）、全图覆盖保持；构造"目标簇横跨中缝"分布时被切目标中心 21/60 → 0/60；
- 真实小集：被切缝劈中的目标 OFF=16 → ON=2（**减少 88%**）；
- 端到端：`slice_prob=1.0 + center_bias=True/False` 各 1 epoch 跑通，`Online augment: 80 training samples from 20 images (4 slices per image)`，len 恒定 4N=80。
...
- **拖影方向**：`blur_axis_aligned`（默认 `True`）把拖影限制为**像面轴对齐**（水平/垂直二选一），适用于相机安装固定、运动方向在像面上恒定映射到水平或垂直的场景（车载 / 航拍沿航迹）；这类域里轴对齐是更贴合的建模。该路径与任意角路径**逐位等价**（轴对齐时 PSF 恰好就是均匀箱式），但走 `cv2.blur` 的 O(1) 行程和实现，实测两档合计 28.1→15.4 ms/图（**1.82×**，长档 1.96×）。需要任意角（方向确实随机的场景）时设 `False` 回退，代价是每图多约 12.7 ms。
...
- **训练后期统一关闭（`close_aug_epoch`，与 `close_mosaic` 同构的时间维调度）**：`close_aug_epoch=N` 时，`set_epoch(epoch, epochs)` 在 `epoch >= epochs - N` 阶段把切片/合成/比例/模糊/气象退化/遮挡六个掩码全部置为**全 False**——各区段"位置保留、内容替换"退回原图直通（len 恒定，与方案 A 同构），模型在最后 N 个 epoch 只在真实样本分布上微调收敛，缓解小数据集后期过拟合到增强分布。`0`=不启用（默认，完全向后兼容）。实现点：`base.py: set_epoch(epoch, epochs=None)` 增加 close 分支；`trainer.py` 调用改为 `set_epoch(epoch, self.epochs)`（dataset 不感知训练调度，由 trainer 传入）。

### 3.6 在线遮挡模拟（occlusion，方案 2）

- **实现**：`base.py` 的 `_occlusion_at` + `_apply_occlusion`。
...
- **颜色**：`occlusion_color=auto`（默认）采样图像暗 25% 分位均值作为块色，融入场景而非突兀黑块；也可固定 `black`/`gray`。
- **开关与比例**：`occlusion_keep`（独立开关，不依赖切片/keep_origin）+ `occlusion_ratio`（原图级，默认 0.5）——每 epoch 选 `round(x×N)` 张做遮挡，未选中整图直通（len 恒定）；与 blur/weather 同构，`close_aug_epoch` 联动关闭。
- **区段**：`[8N+⌈N/4⌉+N, +N)` 在 weather 段之后；`_segment_bases` 升为 7 元组，`__len__` 加 N；N=4 全开 = 16+4+4+8+1+4+4 = **41 张**混合样本池。
- **保存**：`occlusion_save_dir`（指定才保存，画框/按图+类型跨 epoch 去重/`slice_save_max_occlusion` 限数量）。

### 3.7 验证侧在线切片评估（val_slice_*，SAHI 评估）
...
- 实现：`trainer.validate()` 主验证前若 `val_slice_enable` 显式 `validator.args.val_slice_enable=True + dataloader=None`（**强制经 `DetectionValidator.get_dataloader` 重建为切片 loader**——trainer 预建的 `test_loader` 是整图 loader，不重建则 val_slice 不生效）；dual 第二遍临时 `args.val_slice_enable=False + dataloader=None` 跑整图验证（`_val_slice_active()` 动态读 args），随后恢复。`BaseValidator.__call__` 的 dataloader 重建已改为无条件（`dataloader is None` 即经 `get_dataloader` 重建，训练/非训练分支统一）；`model.val()` 默认 `rect=True` 与切片推理冲突（labels 无真实 shape → 空 tile），val_slice active 时自动强制 `rect=False` 重建；
...
- **实现**：`base.py` 的 `_weather_at` + `_apply_weather`（纯内存生成，无离线工具）；区段插在合成段之后（`[8N+ceil(N/4), +N)`）。
...
- **三种退化（强度每次调用随机采样，避免过拟合单一强度）**：
  - **雨**：`weather_rain_density`（雨线数量 ≈ density×max(h,w)）+ `weather_rain_length`（线长上限，每根随机取 0.5~1.0 倍），固定 20° 斜向、半透明叠加（alpha 0.2~0.6）；
  - **雾**：`weather_haze_beta`（[0,1)，越大雾越浓），大气散射模型 `I = J×(1-β) + A×β`，A=灰色大气光 200；
  - **噪声**：`weather_noise_std`（每通道独立高斯噪声），模拟传感器/弱光噪点。
- **比例**：`weather_ratio`（原图级，默认 0.5）——每 epoch 选 `round(x×N)` 张做退化，未选中整图直通（len 恒定）。**权衡**：增强分布过宽会拖累干净场景精度，推荐 0.3~0.6。
- **保存**：`weather_save_dir`（指定才保存，画框/限数量/按图+类型跨 epoch 去重，同 blur_save_dir 模式；上限 `slice_save_max_weather`，回退 `slice_save_max`）。
...
- **读 `hyp` 的在线配置键必须走 `_hyp_get`**：`v8_transforms` 原先用 `getattr(hyp, "<key>")`（**不带默认值**）读取 60 处在线配置键——它与 `hyp.<key>` 完全等价（Ruff `B009` 的判词就是"并不比属性访问更安全"），所以当 `hyp` 不是**当前** `DEFAULT_CFG` 的派生对象时（第三方构造的 namespace、手工 `dict`、或从旧 `args.yaml` / 旧 checkpoint 的 `train_args` 恢复），会在**数据增强构造阶段**抛 `AttributeError` 而整个训练起不来。现统一走 `_hyp_get(hyp, key)`：`hyp` 属性 → 显式 default → `DEFAULT_CFG_DICT[key]`（即 `default.yaml`，默认值的唯一真源）；键在 `hyp` 和 `default.yaml` 里都没有时抛 `ValueError` 并指名该键。于是"**新增配置键要同时改 `default.yaml` + `cfg/__init__.py` + 实际传递点**"这条约定由**代码强制**，不再只靠注释提醒。上游 YOLO 键（`hyp.mosaic` / `hyp.mixup` …）仍是直接属性访问——缺这些的 `hyp` 是真的坏了。
- **区段长度的单一真源与 `len` 漂移防护**：7 个区段的长度只在 `_segment_lengths()` 里算一次，`_segment_bases()` 用**同一个返回值**同时派生「累积边界 + `total`」和「`_seg_cache` 的**版本戳**」（`tuple(段长)`）。两条历史教训：①`__len__` 曾自己重算一遍总长（两处不一致即**静默**丢样本 / 索引越界），已收敛到单一真源；②缓存曾用"首次调用即固化"，而分支开关是在 `__init__` **之后**由 `v8_transforms` 挂上的，任何早于挂载点的 `len()` 都会把旧边界永久固化 —— 改版本戳后这条时序不再有正确性含义。**新增一个区段只需改 `_segment_lengths` 一处**；**不要**另立一份开关清单当缓存键（那就把漂移搬到了下一层）。
...
| `ultralytics/data/augment.py` | `OnlineSlice` 类、Mosaic 保存、独立开关装配、`_hyp_get`（读 `hyp` 的在线配置键，缺失则回退 `default.yaml`）、`reset_counters`、`slice_geometry`（训练/验证切片共用几何） |
| `ultralytics/data/base.py` | 索引空间扩展、`_segment_lengths`（7 区段**长度的单一真源**，同时充当 `_seg_cache` 的**版本戳**，见 §5.5）、`set_epoch` 掩码、`_origin_at` / `_ratio_at` / `_blur_at` / `_compose_at`、`_extended_pool_on`、buffer 记账、样本数打印、worker LRU、`SliceValDataset`（验证侧切片数据集） |
